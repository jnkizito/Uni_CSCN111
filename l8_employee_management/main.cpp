...
int sequentialSearch(string emp[ROW][MAX_EMPLOYEES], int length, string searchName) {
...
void calculateWages(string emp[ROW][MAX_EMPLOYEES]) {
    ...
    for (int i = 0; i < MAX_EMPLOYEES; i++) {
        if (emp[0][i] != " " && emp[0][i] != "") {
            ...
            wageFile << emp[0][i] << " (" << emp[1][i] << ") earned $" << wage << endl;
        }
    }
    wageFile.close();
}
...
int main() {
    ...
    while (true) {
        ...
        if (choice == "H" || choice == "h") hireEmployee(emp);
        ...
        else if (choice == "W" || choice == "w") calculateWages(emp);
        else if (choice == "L" || choice == "l") printEmployeeList(emp);
